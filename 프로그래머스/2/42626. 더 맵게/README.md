...
### 4. 힙 (Heap)
#### 이진트리
- 한 노드가 최대 두개 노드를 자식으로 가질 수 있는 트리
- **`완전이진트리`**
    - 마지막 레벨을 제외한 모든 레벨에 자식 노드가 최대
    - 마지막 레벨의 노드들이 왼쪽부터 순서로 들어가 있는 형태의 트리
    - **특징**
        - 현재 노드 = **`i`**
            - parent node 번호 = **`( i - 1 ) /2`**
            - left child 번호 = **`i * 2 + 1`**
            - right child 번호 = **`i * 2 + 2`**

#### 힙 (Heap)
- 완전 이진 트리 구조
- 부모 노드가 가진 값은 자식 노드의 값보다 무조건 `크다(Max Heap)` 또는 `작다(Min Heap)`
- 가장 큰 값이나 가장 작은 값을 구하고자 할 때 Root 값을 가져오면 되기 때문에 `O(1)` 시간 복잡도를 가진다.
<img width="70%" alt="minHeap_img" src="https://github.com/user-attachments/assets/a35fc5b3-f491-4c5b-9cf3-a3905104726a" />
